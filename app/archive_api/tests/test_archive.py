...
class TestDateValidation:
    ...
    def test_invalid_from_format(self, client, from_date):
        c, _ = client
        resp = c.get(f"/archive?from={from_date}&to=2025-01-31")
        assert resp.status_code == 400
        assert "from" in resp.get_json()["error"]
    ...
    def test_invalid_to_format(self, client, to_date):
        c, _ = client
        resp = c.get(f"/archive?from=2025-01-01&to={to_date}")
        assert resp.status_code == 400
        assert "to" in resp.get_json()["error"]

    def test_from_after_to(self, client):
        c, _ = client
        resp = c.get("/archive?from=2025-01-31&to=2025-01-01")
        assert resp.status_code == 400
        assert "error" in resp.get_json()

    def test_range_exactly_31_days(self, client):
        c, _ = client
        resp = c.get("/archive?from=2025-01-01&to=2025-01-31")
        assert resp.status_code == 200

    def test_range_exceeds_31_days(self, client):
        c, _ = client
        resp = c.get("/archive?from=2025-01-01&to=2025-02-01")
        assert resp.status_code == 400
        assert "31" in resp.get_json()["error"]

    def test_same_day_range(self, client):
        c, _ = client
        resp = c.get("/archive?from=2025-01-15&to=2025-01-15")
        assert resp.status_code == 200
...
class TestArchiveListing:
    ...
    def test_streams_across_months(self, client):
        ...
        resp = c.get("/archive?from=2025-12-31&to=2026-01-01")
        assert resp.status_code == 200
        data = resp.get_json()
        assert "2025" in data
    ...
    def test_streams_outside_range_excluded(self, client):
        ...
        resp = c.get("/archive?from=2025-01-10&to=2025-01-20")
        assert resp.status_code == 200
        data = resp.get_json()
